...
fn paint_overlay(hwnd: HWND) {
    ...
    unsafe {
        ...
        if !mem_dc.is_invalid() {
            ...
            let _ = BitBlt(hdc, 0, 0, state.width, state.height, Some(mem_dc), 0, 0, SRCCOPY);
            if !old.is_invalid() {
...
pub fn show(space: ScreenSpace) -> Result<(), String> {
    ...
    unsafe {
        ...
        *overlay_state()
            .lock()
            .map_err(|_| "native freeze overlay lock poisoned".to_string())? = Some(state);
...
pub fn close() {
    let state = overlay_state().lock().ok().and_then(|mut guard| guard.take());
    ...
    unsafe {
        ...
        let bitmap = HBITMAP(state.bitmap as *mut _);
        if !bitmap.is_invalid() {
            let _ = DeleteObject(HGDIOBJ(bitmap.0));
        }
    }
...
pub fn capture_active_region_to_png(
    ...
    let state = (*guard).ok_or_else(|| "native freeze overlay is not active".to_string())?;
...
unsafe fn crop_bitmap_to_rgba(
    ...
    let copied = BitBlt(crop_dc, 0, 0, crop_w, crop_h, Some(src_dc), crop_x, crop_y, SRCCOPY);
