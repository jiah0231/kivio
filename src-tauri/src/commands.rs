...
use crate::api::{
    call_openai_text, effective_retry_attempts, models_url_from_provider_url,
    resolve_provider_credentials, send_with_failover, send_with_retry, ProviderConnectionInput,
};
...
#[tauri::command]
pub(crate) fn take_translator_selection(state: State<'_, AppState>) -> Result<String, String> {
    take_lens_selection(state)
}
...
pub(crate) async fn fetch_models(
    ...
    let is_claude = base_url.to_ascii_lowercase().contains("/messages");
    ...
    let response = send_with_failover(
        ...
        &api_keys,
        |key| {
            let req = state.http.get(url.clone());
            if is_claude {
                req.bearer_auth(key)
                   .header("anthropic-version", "2023-06-01")
                   .send()
            } else {
                req.bearer_auth(key).send()
            }
        },
    )
    .await?;
